...
MCPForge is a MCP server builder for AI platform engineers, developer-tool teams, internal automation owners. The product must support the complete path from setup to reviewed output, with enough traceability and operational control for real use by a small team.

## 2. Goals

- Deliver one reliable end-to-end workflow for MCP server builder.
...
- connector scaffolding, tool schema generation, sandboxed test calls, manifest validation, release packaging
...
- The product must provide a guided intake form for MCP server builder.
...
- The product should support at least one integration handoff in MVP: GitHub, npm or PyPI registry, OpenAI/Codex clients, local secrets manager.
...
Primary risk areas: tool execution safety, credential handling, schema drift, untrusted generated code.
